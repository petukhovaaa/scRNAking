...
seurat_mat  <- IntegrateLayers(object = seurat_mat, assay = 'RNA', method = CCAIntegration, orig.reduction = "pca", new.reduction = "integrated.cca", verbose = T, k.weight = 30)

# re-join layers after integration
seurat_mat[["RNA"]] <- JoinLayers(seurat_mat[["RNA"]])

seurat_mat  <- FindNeighbors(seurat_mat , reduction = "integrated.cca", dims = 1:pc_num)
seurat_mat  <- FindClusters(seurat_mat , resolution = res)

seurat_mat  <- RunUMAP(seurat_mat , reduction = "integrated.cca", dims = 1:pc_num,reduction.name = 'umap.integrated')
...
DimPlot(seurat_mat, reduction = "umap.integrated",label = T,raster = F) + DimPlot(seurat_mat, reduction = "umap.integrated",label = F,raster = F, group.by = 'orig.ident')
dev.off()
